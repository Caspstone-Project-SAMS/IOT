...
bool connectWebSocket() {
  ...
  return websocketClient.connect("wss://34.81.223.233/ws/module?key=" + key);
}
...
int uploadFingerprintTemplate(String fingerprintTemplate){
  ...
  delay(1);
  http.begin(*client, "https://" SERVER_IP "/api/Fingerprint");
...
int uploadUpdatedFingerprintTemplate(String fingerprintTemplate, uint16_t fingerprintId, String& error){
  ...
  http.setTimeout(2000);
  delay(1);
  http.begin(*client, "https://" SERVER_IP "/api/Fingerprint/update");
  http.addHeader("Content-Type", "application/json");
  int httpCode = http.POST(payload);
  String payloadData = http.getString();
  http.end();
